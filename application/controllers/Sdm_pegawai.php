...
class Sdm_pegawai extends CI_Controller {
	function __construct() {
        parent::__construct();
        $this->load->model("M_sdm_non","sdm_pegawai_non_ciriajasa");
    }

    public function index(){
    	$this->main->check_session();
		$url 				= $this->uri->segment(1);
		$module 			= "sdm_pegawai_non_ciriajasa";
		...
		$data["title"] 		= $this->lang->line('lb_sdm_pegawai_non_ciriajasa');
		$data['url']		= $url;
		$data['module']		= $module;
		$data['id']			= $SDMID;
		$data['content'] 	= 'backend/sdm_pegawai_non_ciriajasa/list';
		$data['form']		= 'backend/sdm_pegawai_non_ciriajasa/form';
		$data['filter']		= 'backend/sdm_pegawai_non_ciriajasa/filter';
		$data['import']		= 'backend/sdm_pegawai_non_ciriajasa/import';
	...
	public function list(){
		$data 	= array();
		$list 	= $this->sdm_pegawai_non_ciriajasa->get_datatables();
		...
		$response = array(
			...
			"recordsTotal" 		=> $this->sdm_pegawai_non_ciriajasa->count_all(),
			"recordsFiltered" 	=> $this->sdm_pegawai_non_ciriajasa->count_filtered(),
			"data" 				=> $data,
		);
	...
	public function edit(){
		...
		$ID 		= $this->input->post('ID');
		$list 		= $this->sdm_pegawai_non_ciriajasa->get_by_id($ID);
	...
	public function active(){
		...
		if(!in_array($HakAksesType, array(1,2))):
			$this->main->general_update("mt_sdm_pegawai_non_ciriajasa", $data, array("sdm_pegawai_non_ciriajasaID" => $ID, "CompanyID" => $CompanyID));
		else:
			$this->main->general_update("mt_sdm_pegawai_non_ciriajasa", $data, array("sdm_pegawai_non_ciriajasaID" => $ID));
		endif;

		if($Active == 1):
			$this->main->inser_log(1,6,'sdm_pegawai_non_ciriajasa', $ID);//$LogType,$Type,$Page,$Content=""
		else:
			$this->main->inser_log(1,7,'sdm_pegawai_non_ciriajasa', $ID);//$LogType,$Type,$Page,$Content=""
		endif;
