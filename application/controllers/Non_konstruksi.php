...
class Non_konstruksi extends CI_Controller {
	function __construct() {
        parent::__construct();
        $this->load->model("M_non_konstruksi","non_konstruksi");
    }

    public function index(){
    	$this->main->check_session();
		$url 				= $this->uri->segment(1);
		$module 			= "non_konstruksi";
		...
		$ID = $this->main->generate_code_non_konstruksi();

		$data["title"] 		= $this->lang->line('lb_non_konstruksi');
		$data['url']		= $url;
		$data['module']		= $module;
		$data['id']			= $ID;
		$data['content'] 	= 'backend/non_konstruksi/list';
		$data['form']		= 'backend/non_konstruksi/form';
		$data['filter']		= 'backend/non_konstruksi/filter';
		$data['import']		= 'backend/non_konstruksi/import';
		
		$data['b_list']		= 'backend/non_konstruksi/breadcumb-list';
		$data['b_form']		= 'backend/non_konstruksi/breadcumb-form';
	...
	public function list(){
		$data 	= array();
		$list 	= $this->non_konstruksi->get_datatables();
		...
		$response = array(
			...
			"recordsTotal" 		=> $this->non_konstruksi->count_all(),
			"recordsFiltered" 	=> $this->non_konstruksi->count_filtered(),
			"data" 				=> $data,
		);
	...
	public function save(){
		...
		$this->validation->non_konstruksi();
		...
		// print_r($data_konstruksi); exit();
		
		$ID = $this->main->generate_code_non_konstruksi();
		
		$data_konstruksi['ID'] 		= $ID;
		$ID = $this->main->general_save("mt_non_konstruksi", $data_konstruksi);
		...
		$DetID = $this->db->insert_batch('mt_non_konstruksi_det', $data_detail); 
	...
	public function edit(){
		...
		$ID 		= $this->input->post('ID');
		$list 		= $this->non_konstruksi->get_by_id($ID);
	...
	public function active(){
		...
		if(!in_array($HakAksesType, array(1,2))):
			$this->main->general_update("mt_non_konstruksi", $data, array("non_konstruksiID" => $ID, "CompanyID" => $CompanyID));
		else:
			$this->main->general_update("mt_non_konstruksi", $data, array("non_konstruksiID" => $ID));
		endif;

		if($Active == 1):
			$this->main->inser_log(1,6,'non_konstruksi', $ID);//$LogType,$Type,$Page,$Content=""
		else:
			$this->main->inser_log(1,7,'non_konstruksi', $ID);//$LogType,$Type,$Page,$Content=""
		endif;
	...
	public function export_word($p1=""){
		...
		foreach ($Pendidikan as $row4 => $row_pendidikan){
			$data_pendidikan[$row4] = array(
				'Pendidikan' => utf8_decode($row_pendidikan) . " ",
			);
		}
		...
		foreach ($Pendidikan_non as $row3 => $row_pendidikan_non){
			$data_pendidikan_non[$row3] = array(
				'Pendidikan_non_formal' => utf8_decode($row_pendidikan_non) . " ",
			);
		}
	...
	public function export_word2($p1=""){
		$ID 							= $this->input->post("ID");
		$Posisi 						= $this->input->post("Posisi");
		$Nama_perusahaan1 				= $this->input->post("Nama_perusahaan1");
		$Nama_personil 					= $this->input->post("Nama_personil");
	...
	public function template_import(){
		$this->non_konstruksi->export('template');
	}
	...
	private function upload_import(){
		...
		$folder = $this->main->create_folder_general('import_non_konstruksi');
		...
		if (!$this->upload->do_upload('file')):
			...
			for ($row = 2; $row <= $highestRow; $row++){
				...
				if(count($rowData[0]) == 2):
					...
					$ck_code = $this->db->count_all("mt_non_konstruksi where CompanyID = '$CompanyID' and Code = '$Code'");
					if(!$Code):
			...
			for ($row = 2; $row <= $highestRow; $row++){
				...
				if(count($rowData[0]) == 2):
			        ...
			        if($status_product):
			        	...
			        	$this->main->general_save("mt_non_konstruksi", $data_import);
			        endif;
				endif;
			}
