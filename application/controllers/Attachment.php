...
class Attachment extends CI_Controller {
	...
	public function save(){
	    ...
	    {           
			...
			$cek 		= $this->db->count_all("ut_attachment where Type='$Type' $querycek ");
			$CompanyID 	= $this->session->CompanyID;
	...
	public function update_attachment_cek($Type,$ID)
	{
		...
		$this->db->where("ID",$ID);
		$query = $this->db->update("ut_attachment");
	...
	public function delete($AttachmentID){
		$ID = $this->session->ID;
		$cek = $this->db->count_all("ut_attachment where ID = '$ID' and AttachmentID = '$AttachmentID'");
		if($cek>0):
	...
	public function save_new_attachment(){
		...
		if(count($file)>0):
			foreach ($file as $k => $v) {
				...
				$nmfile = "E-Database-CV".time().$k.'.'.$name[1];
	            ...
	            if(file_put_contents($folder.$nmfile,$decoded)):
					$querycek 	= " and ID = '$ID'";
					$cek 		= $this->db->count_all("ut_attachment where ID = '$ID' and Type='$type' $querycek ");
	            	if($cek>0):
	            		$cek = 0;
	            	else:
	            		$cek = 1;
                    ...
                    $this->main->general_save("ut_attachment", $data_atach);
