...
$('.btn-save-demo').click(function () {
  $('.form-group').removeClass('has-error'); // clear error class
  $('.help-block').empty();
  xform = '#loginform';
  $(xform + ' [name=username]').val('admin');
  $(xform + ' [name=password]').val('P@ssword123!');
  proccess_save();
  url = url_string + 'api/login';
  $.ajax({
    url: url,
    ...
    if (data.inputerror) {
      for (var i = 0; i < data.inputerror.length; i++) {
        ...
        if (input_type == 'select2') {
          ...
          $('.' + data.inputerror[i] + '-view .form-control-feedback').text(
            data.error_string[i],
          );
          ...
          $('.' + data.inputerror[i])
            .eq(data.error_string[i])
            .addClass('border-red-validate');
