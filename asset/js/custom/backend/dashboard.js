...
var url_list_ahli   = host + "dashboard-list-ahli";
...
function proyek(data){
    ...
    if (data.total_proyek.length > 1){
        $.each(data.total_proyek,function(i,v){
            ...
            item    =  '<div class="sl-item">\
                            <div class="sl-left"><img class="img-circle" alt="user" src="'+image+'"></div>\
                            <div class="sl-right">\
                                <div class="font-medium">'+data.total_proyek[i].tahun+'</div>\
                                <div class="desc">'+data.total_proyek[i].jumlah_proyek+' Current Project</div>\
                            </div>\
                        </div>';
...
function get_backgroundColor(index,transaparant){
    ...
    // custom
        ...
        backgroundcolor = 'linear-gradient(60deg, rgba(255,175,16,1) 30%, rgba(253,116,155,1) 37%)';
...
function filter_table(){
    ...
    //datatables
    let table = $('#table-daftar-pegawai-pt').DataTable({
        "searching": false,
        ...
        "ajax": {
            ...
            dataSrc : function (json) {
              ...
              // console.log("Table Daftar Pegawai PT", json, table.page.info());
              let infoTable = table.page.info();  

              if(infoTable?.start <= 0){
                $('#table-daftar-pegawai-pt-pagination .pagination .prev-btn').attr('disabled', true);
              }else{
                $('#table-daftar-pegawai-pt-pagination .pagination .prev-btn').removeAttr('disabled');
              }

              if(json?.recordsFiltered <= infoTable?.length || json?.recordsFiltered === infoTable?.end ){
                $('#table-daftar-pegawai-pt-pagination .pagination .next-btn').attr('disabled', true);
              }else{
                $('#table-daftar-pegawai-pt-pagination .pagination .next-btn').removeAttr('disabled');
              }

              if(json?.data?.length > 0){
                $('#table-daftar-pegawai-pt-pagination .display-info').html(`<p>Menampilkan <b>${infoTable?.start + 1}</b> hingga <b>${infoTable?.end <= 0 ? json?.data?.length <= infoTable?.length ?  json?.data?.length : infoTable?.length : infoTable?.end}</b> dari <b>${json?.recordsFiltered}</b> entri</p>`);
                $('#table-daftar-pegawai-pt-pagination .pagination').removeClass('d-none');
                $('#table-daftar-pegawai-pt-pagination .pagination .curr-page').html(`${infoTable?.page + 1}`);
              }else{
                $('#table-daftar-pegawai-pt-pagination .display-info').empty();
                $('#table-daftar-pegawai-pt-pagination .pagination').addClass('d-none');
                $('#table-daftar-pegawai-pt-pagination .pagination .curr-page').empty();
              }
    ...
    $('#table-daftar-pegawai-pt_wrapper .dataTables_paginate, #table-daftar-pegawai-pt_wrapper .dataTables_info').hide();

    $('#table-daftar-pegawai-pt-pagination .pagination > .prev-btn').on('click', function(){
        table.page( 'previous' ).draw( 'page' );
    });

    $('#table-daftar-pegawai-pt-pagination .pagination > .next-btn').on('click', function(){
...
function filter_table2(){
    ...
    //datatables
    let table = $('#table-daftar-pegawai').DataTable({
        "searching": false,
        "processing": true, //Feature control the processing indicator.
        "serverSide": true, //Feature control DataTables' server-side processing mode.
        "destroy":true,
        ...
        "ajax": {
            ...
            dataSrc : function (json) {
              ...
              // console.log("Table Daftar Pegawai", json, table.page.info());
              let infoTable = table.page.info();  

              if(infoTable?.start <= 0){
                $('#table-daftar-pegawai-pagination .pagination .prev-btn').attr('disabled', true);
              }else{
                $('#table-daftar-pegawai-pagination .pagination .prev-btn').removeAttr('disabled');
              }

              if(json?.recordsFiltered <= infoTable?.length || json?.recordsFiltered === infoTable?.end ){
                $('#table-daftar-pegawai-pagination .pagination .next-btn').attr('disabled', true);
              }else{
                $('#table-daftar-pegawai-pagination .pagination .next-btn').removeAttr('disabled');
              }

              if(json?.data?.length > 0){
                $('#table-daftar-pegawai-pagination .display-info').html(`<p>Menampilkan <b>${infoTable?.start + 1}</b> hingga <b>${infoTable?.end <= 0 ? json?.data?.length <= infoTable?.length ?  json?.data?.length : infoTable?.length : infoTable?.end}</b> dari <b>${json?.recordsFiltered}</b> entri</p>`);
                $('#table-daftar-pegawai-pagination .pagination').removeClass('d-none');
                $('#table-daftar-pegawai-pagination .pagination .curr-page').html(`${infoTable?.page + 1}`);
              }else{
                $('#table-daftar-pegawai-pagination .display-info').empty();
                $('#table-daftar-pegawai-pagination .pagination').addClass('d-none');
                $('#table-daftar-pegawai-pagination .pagination .curr-page').empty();
              }
    ...
    $('#table-daftar-pegawai_wrapper .dataTables_paginate, #table-daftar-pegawai_wrapper .dataTables_info').hide();

    $('#table-daftar-pegawai-pagination .pagination > .prev-btn').on('click', function(){
        table.page( 'previous' ).draw( 'page' );
    });

    $('#table-daftar-pegawai-pagination .pagination > .next-btn').on('click', function(){
        table.page( 'next' ).draw( 'page' );
    });
}

function filter_table3(){
    // page_data   = $(".page-data").data();
    // dt_url      = page_data.url;
    // dt_module   = page_data.module;

    var form_filter = '#form-filter3';
    Searchx         = $(form_filter+" [name=f-Search]").val();
    ...
    //datatables
    let table = $('#table-biodata').DataTable({
        "searching": false,
        "processing": true, //Feature control the processing indicator.
        "serverSide": true, //Feature control DataTables' server-side processing mode.
        "destroy":true,
        ...
        "ajax": {
            ...
            "type"  : "POST",
            dataSrc : function (json) {
              // console.log("BIODATA", json, table.page.info());
              let infoTable = table.page.info();  

              if(infoTable?.start <= 0){
                $('#table-biodata-pagination .pagination .prev-btn').attr('disabled', true);
              }else{
                $('#table-biodata-pagination .pagination .prev-btn').removeAttr('disabled');
              }

              if(json?.recordsFiltered <= infoTable?.length || json?.recordsFiltered === infoTable?.end ){
                $('#table-biodata-pagination .pagination .next-btn').attr('disabled', true);
              }else{
                $('#table-biodata-pagination .pagination .next-btn').removeAttr('disabled');
              }

              if(json?.data?.length > 0){
                $('#table-biodata-pagination .display-info').html(`<p>Menampilkan <b>${infoTable?.start + 1}</b> hingga <b>${infoTable?.end <= 0 ? json?.data?.length <= infoTable?.length ?  json?.data?.length : infoTable?.length : infoTable?.end}</b> dari <b>${json?.recordsFiltered}</b> entri</p>`);
                $('#table-biodata-pagination .pagination').removeClass('d-none');
                $('#table-biodata-pagination .pagination .curr-page').html(`${infoTable?.page + 1}`);
              }else{
                $('#table-biodata-pagination .display-info').empty();
                $('#table-biodata-pagination .pagination').addClass('d-none');
                $('#table-biodata-pagination .pagination .curr-page').empty();
              }
    ...
    $('#table-biodata_wrapper .dataTables_paginate, #table-biodata_wrapper .dataTables_info').hide();

    // $('#test-paging').on('click', function(){
    //     table.page( 'next' ).draw( 'page' );
    // });

    $('#table-biodata-pagination .pagination > .prev-btn').on('click', function(){
        table.page( 'previous' ).draw( 'page' );
    });

    $('#table-biodata-pagination .pagination > .next-btn').on('click', function(){
        table.page( 'next' ).draw( 'page' );
    });
}
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            if(data.status){
                ...
                $(xform+' [name=Nama_perusahaan]').val(dt_value.Proyek);
                $(xform+' [name=Peride_proyek_mulai]').val(dt_value.Periode_proyek_mulai);
                $(xform+' [name=Peride_proyek_selesai]').val(dt_value.Periode_proyek_selesai);
