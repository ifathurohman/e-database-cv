...
function open_modal_icon(p1){
	$('.dt-icon-list').data('p1', p1);
...
function set_fa_icon(val){
	$('.dt-icon-list').empty();

	item = '';
	$.each(dt_icon,function(k,v){
		if(k.includes(val) || !val){
			item += '<div class="fa-hover col-md-3">\
            <a href="javascript:;" onclick="choose_icon(this)" data-icon="fa '+k+'">\
	...
	$('.dt-icon-list').append(item);
}

function choose_icon(element){
	tg_dt_icon  = $(element).data();
	tg_dt_icon2 = $('.dt-icon-list').data();

	$('.'+tg_dt_icon2.p1).val(tg_dt_icon.icon);
